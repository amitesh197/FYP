...
opt.detect_parsing_model = os.path.join(root_save_path, 'detect_parsing.pth')
opt.reconstruct_model = os.path.join(root_save_path, 'reconstruction.pth')
...
trained_deoc_model = torch.load(opt.detect_parsing_model, map_location=torch.device('cpu'))['state_dict']
...
trained_deoc_model = torch.load(opt.reconstruct_model, map_location=torch.device('cpu'))['state_dict']
reconstruct_model.load_state_dict(trained_deoc_model)

detect_parsing_model = detect_parsing_model.cpu()
reconstruct_model = reconstruct_model.cpu()
...
for i, fname in enumerate(os.listdir(opt.test_dir)):
    ...
    if is_image_file(fname):
        ...
        for c, m in zip(deoc_img, opt.deoc_norm_mean):
            c.sub_(m)
        img_var = deoc_img.view(1, 3, opt.image_size, opt.image_size).to('cpu')
        ...
        gen_face = reconstruct_model(img_var.to('cpu'), seg_mask.to('cpu'), parsing_mask.to('cpu'))
        gen_face = gen_face[0, :, :, :]
